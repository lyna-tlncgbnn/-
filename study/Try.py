...
def train():
    ...
    if mid.button("训练"):
        ...
        with st.form(key='training'):
            ...
            with col1:
                epoch = st.number_input("epoch",min_value=20,max_value=1000,value=80)
            with col2:
                lr = st.number_input("learning rate(e10-4)",min_value=1,max_value=1000,value=10)
            with col3:
                option = st.selectbox("请选择优化方法",("Adam",'SGD','RMSprop'))
            sub = st.form_submit_button("开始训练")
        if sub:
            pass
        #   train(epoch,lr,option,0)
